...
Flights of the types `COLONIZE`, `TRANSFER` and `TRANSPORT` can carry a specified amount of cargo as shown in the example above. The cargo will be available on the target planet at the beginning of the round in which the ships arrive.
